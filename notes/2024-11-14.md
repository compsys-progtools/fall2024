...
## Control Unit (CU)
Description
    - The CU manages the execution of instructions. It drects the flow of data between the CPU, memory, and I/O devices. It also decodes instructions from the memory and than executes them step by step

How it's built
    - The CU comprises a set of registers, decoders, multiplexers, and a clock to sequence operations.
    Modern implementations might use microprogramming or hardwired logic.
    
Predecessors
    -Manchester Baby: lacked a formal control unit, relying instead on simpler instruction sequencing. John von Neumann's      introduction of the stored-program concept necessitated a dedicated control mechanism.

- [Wikipedia](https://en.wikipedia.org/wiki/Control_unit)  
- [Britannica](https://www.britannica.com/technology/control-unit)
